...
3. When a refresh is needed, run the repository's Playwright setup test from the repo root.
4. Prefer the repo's package manager, run from the `apps/playwright-tests/` directory:

   ```bash
   NODE_OPTIONS='--env-file=.env' pnpm playwright test setup.spec.ts
   ```
