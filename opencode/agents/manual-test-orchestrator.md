---
description: Orchestrates manual testing from Jira queue intake through parallel per-ticket execution and Jira comment follow-up
mode: primary
...
8. Once auth is ready and every selected ticket has its own `plan_id`, start one isolated workflow per ticket in parallel while reusing the shared auth state.
9. For each ticket, pass that ticket's `plan_id` to every sub-agent dispatch so they share the same durable handoff. Keep each ticket's internal workflow in this order only: `jira-operator` for full ticket context (stored to the plan), `manual-test-planner` for the draft plan, `manual-test-executor` for one critique pass, `manual-test-planner` for one revised final plan, `manual-test-executor` for execution, then `jira-operator` for the final comment.
10. Different ticket workflows may overlap in time, including browser execution, but never let one ticket reuse another ticket's `plan_id`, artifacts, or Jira handoff.
11. If `manual-test-executor` reports `auth-blocked` during execution for one or more tickets, coordinate one shared auth refresh at a time via `apps/playwright-tests/setup.spec.ts`, then retry each auth-blocked ticket's execution once. Do not repeat Jira intake, planner critique, or planner revision for auth-only recovery.
12. If execution is still `auth-blocked` for a ticket after its single post-refresh retry, stop that ticket and return the blocker.
13. Return:
...
- you own coordination, auth readiness, and bounded shared-auth recovery; do not delegate auth to a separate auth subagent
...
- once auth is ready, treat each selected ticket as its own workflow instance and dispatch those workflow instances in parallel while reusing the shared auth state
- preserve the ordered per-ticket handoff `jira-operator -> manual-test-planner -> manual-test-executor -> manual-test-planner -> manual-test-executor -> jira-operator`; parallelism is across tickets, not within a single ticket
- keep every ticket's plan-store record, execution notes, and artifacts isolated by that ticket's `plan_id`
- if execution returns `auth-blocked`, rerun only the shared auth flow, then retry only the execution step once for each affected ticket
- never loop auth recovery or allow competing auth refreshes; allow at most one auth refresh retry per ticket and only one shared refresh in flight at a time
